...
import software.amazon.sagemaker.modelqualityjobdefinition.Utils;
...
public class ReadHandler extends BaseHandlerStd {
    ...
    protected ProgressEvent<ResourceModel, CallbackContext> handleRequest(
        ...
        if(StringUtils.isEmpty(jobDefinitionName)){
            jobDefinitionName = Utils.getResourceNameFromArn(model.getJobDefinitionArn(), MODEL_QUALITY_ARN_SUBSTRING);
            model.setJobDefinitionName(jobDefinitionName);
        }

        return proxy.initiate("AWS-SageMaker-ModelQualityJobDefinition::Read", proxyClient, model, callbackContext)
     ...
     */
    private DescribeModelQualityJobDefinitionResponse readResource(
            final DescribeModelQualityJobDefinitionRequest awsRequest,
            final ProxyClient<SageMakerClient> proxyClient,
            final ResourceModel model) {

        DescribeModelQualityJobDefinitionResponse response = null;
        try {
            response = proxyClient.injectCredentialsAndInvokeV2(awsRequest, proxyClient.client()::describeModelQualityJobDefinition);
    ...
    private ProgressEvent<ResourceModel, CallbackContext> constructResourceModelFromResponse(
            final DescribeModelQualityJobDefinitionResponse awsResponse) {
