...
public class CreateHandler extends BaseHandlerStd {
    ...
    protected ProgressEvent<ResourceModel, CallbackContext> handleRequest(
        ...
        return ProgressEvent.progress(model, callbackContext)
            .then(progress ->
                    proxy.initiate("AWS-SageMaker-ModelQualityJobDefinition::Create", proxyClient, model, callbackContext)
     ...
     */
    private CreateModelQualityJobDefinitionResponse createResource(
            final CreateModelQualityJobDefinitionRequest awsRequest,
            final ProxyClient<SageMakerClient> proxyClient) {

        CreateModelQualityJobDefinitionResponse response = null;
        try {
            response = proxyClient.injectCredentialsAndInvokeV2(awsRequest, proxyClient.client()::createModelQualityJobDefinition);
