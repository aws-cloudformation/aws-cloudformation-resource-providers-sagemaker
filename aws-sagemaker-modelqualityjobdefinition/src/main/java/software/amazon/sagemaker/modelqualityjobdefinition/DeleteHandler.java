...
public class DeleteHandler extends BaseHandlerStd {
    ...
    protected ProgressEvent<ResourceModel, CallbackContext> handleRequest(
        ...
        return ProgressEvent.progress(model, callbackContext)
                .then(progress ->
                        proxy.initiate("AWS-SageMaker-ModelQualityJobDefinition::Delete", proxyClient, model, callbackContext)
     ...
     */
    private DeleteModelQualityJobDefinitionResponse deleteResource(
            final DeleteModelQualityJobDefinitionRequest awsRequest,
            final ProxyClient<SageMakerClient> proxyClient) {

        DeleteModelQualityJobDefinitionResponse response = null;
        try {
            response = proxyClient.injectCredentialsAndInvokeV2(awsRequest, proxyClient.client()::deleteModelQualityJobDefinition);
