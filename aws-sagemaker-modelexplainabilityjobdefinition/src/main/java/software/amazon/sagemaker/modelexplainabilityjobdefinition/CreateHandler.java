...
public class CreateHandler extends BaseHandlerStd {
    ...
    protected ProgressEvent<ResourceModel, CallbackContext> handleRequest(
        ...
        return ProgressEvent.progress(model, callbackContext)
            .then(progress ->
                    proxy.initiate("AWS-SageMaker-ModelExplainabilityJobDefinition::Create", proxyClient, model, callbackContext)
     ...
     */
    private CreateModelExplainabilityJobDefinitionResponse createResource(
            final CreateModelExplainabilityJobDefinitionRequest awsRequest,
            final ProxyClient<SageMakerClient> proxyClient) {

        CreateModelExplainabilityJobDefinitionResponse response = null;
        try {
            response = proxyClient.injectCredentialsAndInvokeV2(awsRequest, proxyClient.client()::createModelExplainabilityJobDefinition);
