...
public class ExceptionMapper {
    ...
    static void throwCfnException(final String operation, final String resourceType, final String resourceName, final AwsServiceException e) {
        if (StringUtils.isNotBlank(e.getMessage())
                && (e.getMessage().matches(".*Cannot find Model Package Group:.*")
                || e.getMessage().matches(".*ModelPackageGroup .* does not exist.*"))) {
            throw new CfnNotFoundException(resourceType, resourceName, e);
        }
        if (StringUtils.isNotBlank(e.getMessage()) && e.getMessage().matches(".*Model Package Group already exists.*")) {
