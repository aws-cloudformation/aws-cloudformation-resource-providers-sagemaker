...
public class CreateHandler extends BaseHandlerStd {
    ...
    private static final String OPERATION = "AWS-SageMaker-UserProfile::Create";
    ...
    protected ProgressEvent<ResourceModel, CallbackContext> handleRequest(
        ...
        if (callbackContext.callGraphs().isEmpty()) {
            if (model.getUserProfileArn() != null) {
                throw new CfnInvalidRequestException(String.format(READ_ONLY_PROPERTY_ERROR_MESSAGE, "UserProfileArn"));
            }
        }
     ...
     */
    private CreateUserProfileResponse createResource(
            final CreateUserProfileRequest createRequest,
            final ProxyClient<SageMakerClient> proxyClient) {
        CreateUserProfileResponse response = null;
        try {
            response = proxyClient.injectCredentialsAndInvokeV2(
                    createRequest, proxyClient.client()::createUserProfile);
        } catch (final AwsServiceException e) {
            Translator.throwCfnException(Action.CREATE.toString(), ResourceModel.TYPE_NAME, createRequest.userProfileName(), e);
        }
        return response;
    }
    ...
    private ProgressEvent<ResourceModel, CallbackContext> constructResourceModelFromResponse(
            final ResourceModel model, final CreateUserProfileResponse awsResponse) {
